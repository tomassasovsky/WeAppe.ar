...
class InviteCreateMiddleware extends Middleware<InviteCreateMiddleware> {
  ...
  FutureOr<void> defineVars(HttpRequest req, HttpResponse res) async {
    organizationName = await InputVariableValidator<String>(
      req,
      'organizationName',
    ).required();

    recipientEmail = await InputVariableValidator<String>(
      req,
      'recipientEmail',
      regExp: Validators.emailRegExp,
    ).required();

    message = await InputVariableValidator<String>(
      req,
      'message',
    ).optional();

    rawUserType = await InputVariableValidator<String>(
      req,
      'userType',
    ).optional();
  ...
  FutureOr<dynamic> run(HttpRequest req, HttpResponse res) async {
    ...
    final organization = await Organization.generic.findOne(
      where.eq('name', organizationName).and(
            where.eq('employers', userId).or(where.eq('admin', userId)),
          ),
    );
    ...
    final userWithEmail = await User.generic.findOne(
      where.eq('email', recipientEmail),
    );

    if (userWithEmail != null) {
      if ((organization.employees ?? []).contains(userWithEmail.id) &&
      ...
      if ((organization.employers ?? []).contains(userWithEmail.id) &&
